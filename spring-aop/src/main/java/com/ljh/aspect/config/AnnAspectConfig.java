 ...
 * @Pointcut("@annotation(com.ljh.anno.AdminOnly) && within(com.ljh..*)")
 * 
 * 匹配标注有 NeedSecured 的类底下的方法，要求 RetentionPolicy 级别为 class 级别
 * @Pointcut("@within(com.ljh.anno.NeedSecured) && within(com.ljh..*)")
 * 
 * 匹配标注有 NeedSecured 的类及其子类的方法，要求 RetentionPolicy 级别为 runtime 级别
 * 在 spring context 的环境下，二者没有区别
 * @Pointcut("@target(com.ljh.anno.NeedSecured) && within(com.ljh..*)")
 * 
 * 匹配传入的参数类标注有 Repository 注解的方法
 * @Pointcut("@args(com.ljh.anno.NeedSecured) && within(com.ljh..*)")
 * 
 * @author Arsenal
